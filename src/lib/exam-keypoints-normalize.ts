...
export function questionTotalPoints(
  question: {
    keyPoints?: { defaultGrade: number }[];
...
/**
 * يحمى سقف الاختبار الإجمالي. إذا تجاوز مجموع كل الأسئلة سقف الاختبار (totalGrade)،
 * يتم ضبط كل الأسئلة والمحاور بداخلها تناسبياً.
 */
export function scaleAllQuestionsToTotal<
  Q extends { questionMaxPoints?: number; keyPoints: { defaultGrade: number; point?: string }[] }
