...
import { isTransientNetworkError, sleep } from "@/lib/ai-helpers";
...
const STUDENT_AI_MODELS = dedupeModels(
  String(process.env.STUDENT_AI_MODELS || "").trim()
    ? String(process.env.STUDENT_AI_MODELS || "")
        .split(",")
...
function makeStudentPromptRules(): string {
  return `${VERBATIM_RULES}
...
Rules:
${makeStudentPromptRules()}

Return ONLY raw JSON:
{"questionNumber": ${questionNumber}, "questionText": "string", "studentAnswer": "string"}`;
}

function buildBatchPrompt(batch: Array<{ id: number; label?: string; text?: string }>): string {
  return `You are an expert OCR assistant.
...
export function createStudentExtractionProviderRuntimes(): StudentProviderRuntime[] {
  ...
  try {
    providers = aiManager.getProviderChain();
  } catch (error) {
    throw createStudentFailure({
      fallbackMessage:
        "No API AI provider is configured for student extraction. Configure AI_PROVIDER with xai, openai, or custom and add the matching API key.",
      statusCode: 503,
      code: "NO_PROVIDER",
      cause: error,
    });
  ...
  const runtimes = providers
    .map((provider) => ({
      provider,
      label: provider.name,
      models: modelsForProvider(provider.name),
    }))
