...
} from "@/modules/exams/lib/exam-scoring";
import { sortExamQuestionsForDisplay } from "@/modules/exams/lib/exam-question-order";
import { isPrimarilyEnglish } from "@/common/lib/text-direction";
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-8 pb-16">
      ...
      {exam.status === "REJECTED" && (
        <section
          className="rounded-xl border border-[#D32F2F]/30 bg-[#FFEBEB] p-5 text-right shadow-sm dark:bg-[#2A1616] dark:border-[#EF5350]/30"
          role="region"
          aria-label="سبب الرفض"
        >
          <h2 className="mb-3 flex flex-wrap items-center gap-2 text-sm font-bold text-[#D32F2F] dark:text-[#EF5350]">
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-baseline justify-between gap-3 border-b border-border/60 pb-3">
                  ...
                  <span className="tabular-nums text-sm font-semibold text-brand-teal">
                ...
                {q.teacherNote?.trim() ? (
                  <div className="mt-4 rounded-lg border border-brand-orange/30 bg-brand-orange/10 px-4 py-3">
                    <p className="mb-1 text-[11px] font-semibold text-brand-orange">
                      توجيهات إضافية للمراجعة
                    </p>
                    <p className="text-sm leading-relaxed text-brand-orange">
                      {q.teacherNote}
                    </p>
                  </div>
                ) : null}
                      ...
                      {q.keyPoints.map((kp: any, ki: number) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="shrink-0 tabular-nums text-sm font-semibold text-brand-teal">
