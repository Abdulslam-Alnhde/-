...
import { distributeEqualAmongKeyPoints } from "@/modules/exams/lib/exam-scoring";
...
export type ExtractedQuestionType = "RUBRIC" | "OBJECTIVE";
...
export const useExamStore = create<ExamState>((set) => ({
  ...
  updateKeyPointGrade: (qIndex, pIndex, grade) => set((state) => {
    const newQuestions = JSON.parse(JSON.stringify(state.extractedQuestions));
    if (newQuestions[qIndex]?.keyPoints?.[pIndex]) {
