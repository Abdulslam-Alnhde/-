...
async function main() {
  // ✅ فحص الوجود — تجنب مسح البيانات الحقيقية عند إعادة التشغيل العرضية
  // لإعادة التهيئة عمداً: شغّل  npm run db:seed:force
  const forceReset = process.env.FORCE_SEED === "true";
  if (!forceReset) {
    const existingAdmin = await prisma.user.findFirst({ where: { role: "ADMIN" } });
    if (existingAdmin) {
      console.log("✅ قاعدة البيانات تحتوي على بيانات بالفعل — تخطّي الـ seed.");
      console.log(`   المشرف الموجود: ${existingAdmin.email}`);
      console.log("   لإعادة التهيئة الكاملة عمداً، شغّل: npm run db:seed:force");
      return;
    }
  } else {
    console.log("⚠️  FORCE_SEED=true — إعادة تهيئة كاملة مقصودة…");
  }

  console.log("🗑️  حذف جميع البيانات (قاعدة البيانات فارغة — بدء التهيئة)…");
